...
import {ReporterOptions} from './reporter/reporter';

// jsonifyNotice serializes notice to JSON and truncates params,
// environment and session keys.
export default function jsonifyNotice(notice: Notice, opts: ReporterOptions, maxLength = 64000): string {
    let s = '';
    for (let level = 0; level < 8; level++) {
        ...
        let escaped = JSON.parse(JSON.stringify(notice)); // escape Notice interface
    ...
    let err = new Error(
        `faultline-js: cannot jsonify notice (length=${s.length} maxLength=${maxLength})`);
    (err as any).params = {
